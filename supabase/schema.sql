...
-- Notes are individual documents that can be filed or left uncategorised.
...
-- One source of truth for Free and paid Filing Room limits.
...
as $$
begin
  if target_plan in ('pro', 'classroom_pro') then return null; end if;
  if target_plan <> 'free' then raise exception 'Unknown plan'; end if;
  case limit_kind
...
begin
  ...
  from public.profiles p where p.id = new.user_id for update;
...
-- Notes: each user can access their own note documents.
